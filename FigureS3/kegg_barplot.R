...
#
# 输入文件为功能丰度文件(如kegg_profile.txt)+分组信息(group.txt)
#
# 输入文件"-i", "--input"，功能丰度文件kegg_profile.txt;
#
...
#
# 分组列名"-o", "--output"，输出目录，默认同输入+.kegg_barplot.pdf；
#
...
if (TRUE){
  option_list = list(
    make_option(c("-i", "--input"), type="character", default="kegg_profile.txt",
                help="kegg profile [default %default]"),
	make_option(c("-l", "--level"), type="character", default="level1",
				help="kegg level [default %default]"),
...
# 设置输出文件缺省值，如果为空，则为输入+kegg_barplot.pdf
if(opts$output==""){opts$output=paste0(opts$input,".kegg_barplot.pdf")}
